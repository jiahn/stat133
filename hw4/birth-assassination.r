...
generateKids <- function(lambda, kappa, parent) {
    ...
    birthdates = runif( rpois(1,parent.life*lambda) , min = parent$birth.date , max = parent$assassination.date )
    n.kids = length(birthdates)
    kids = parent
    
    #id = parent$child.id 
    deathdates = sapply(parent$assassination.date, function(add) add + rexp(n.kids, kappa))
    
#     birthdates = runif(n.kids+1, parent$birth.date, parent$assassination.date)[-1]
#     deathdates = birthdates + rexp(n.kids, kappa)
    
    if (n.kids) {
      
      for (n.kids in 1:n.kids) {
        kids[n.kids,] = c(parent$child.id, n.kids, birthdates[n.kids], deathdates[n.kids])
        row.names(kids) = NULL
      }
...
nextGeneration <- function(lambda, kappa, parents) {
    ...
    next.gen = vector("list", nrow(parents))
    
    i = nrow(parents)
    for (i in 1:i) {
      
      next.gen[[i]] = generateKids(lambda, kappa, parents[i,])
...
bAGen <- function(lambda, kappa, max.gen) {
    ...
    firstgen = data.frame(parent.id = 0, child.id = 0, birth.date = 0, assassination.date = rexp(1, rate = kappa))
    ...
    for (max.gen in 1:max.gen) {
      
      newgen = nextGeneration(lambda, kappa, firstgen)
      generation.list[[max.gen+1]] = newgen
      
      if (is.null(newgen) == TRUE) {
        stop
      }
...
gensSurvived <- function(bagen.simulation) {
    ...
    sims = length(bagen.simulation)
    generations = c(1:sims)
    
    for (sims in 1:sims) {
      generations[sims] = length(bagen.simulation[[sims]])
    }
...
survive.1 = gensSurvived(sim.1)
survive.2 = gensSurvived(sim.2)
survive.3 = gensSurvived(sim.3)

plot(density(survive.1, bw = 0.35), col = 'red', xlab = "generations survived", main = "B-A Simulation", xlim = c(0,11))
lines(density(survive.2, bw = 0.35), col = 'green')
lines(density(survive.3, bw = 0.35), col = 'blue')
legend("topright", "Red = Sim1, Green = Sim2, Blue = Sim3")
